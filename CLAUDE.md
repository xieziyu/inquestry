...
**动手前先读 [`docs/design/overview.md`](docs/design/overview.md)**：定位、词汇与核心概念，`docs/` 里只有它一份。设计与实现细节一律以代码和它旁边的注释为准。

---

## 文档纪律

**默认不动 `docs/`。** 它只有一份 `overview.md`，只写不随代码变的东西（定位、词汇、核心概念、外部实测的硬约束）。一轮开发的产物是代码和它的注释。
