/**
 * 同时开着的多个调查（D28 / ui.md §8.3）。
 *
 * 切换调查**不中断任何一个**：main 这边持有全部运行时，renderer 只是换个投影看。
...
export type LiveCase = {
  /** 待办数要能单独问：全局汇总每 60ms 就要把所有调查算一遍，走不起 buildSnapshot。 */
...
/** 每个活跃会话背后是一个 spawn 出来的 CLI 进程，所以这一档限的是它，不是载入的调查数。 */
...
export class CaseRegistry<R extends LiveCase> {
  ...
  /**
   * renderer 说的那次调查**仍是当前调查**时才给它 runner。
   *
   * 切调查那一瞬 main 这边当时就换了，而旧界面要等下一次快照（最多 60ms）才换屏——
   * 这中间它发出的调用带的还是旧 caseId。不核对的话，在调查 A 里写了一半的线索
   * 会被投进调查 B 的会话，而且 handler 还会回一个「送到了」把 A 的草稿清掉。
  ...
  /** 新建的调查：运行时由调用方造好再交进来——建单信息只有那一侧有。 */
  ...
  /** 切到某个调查；没载入过就按库里的建单信息重建一个。库里没有这个 id 时返回 false。 */
  ...
  /** 去新建调查面板。**当前调查的运行时不动**：建到一半改主意，回去它还在跑。 */
  toIntake() {
    this.currentId = null;
  }

  /**
   * 彻底忘掉一个调查（删除时用）。与限流那两条降级路径的区别在于**它不挑人**：
   * 正跑着的、挂着待办的、当前那个，一律收掉——人已经决定这次调查整个不要了。
   *
   * 🔴 **删库之前必须先走这一下。** 留着运行时的话，那个 runner 手上还攥着一个活的
   * 会话：它下一条消息会往一个已经不存在的 case 上写事件，于是被删掉的调查
   * 自己长回来一半（有事件、没有 `cases` 行），而这一路上没有任何报错。
   *
   * 删的是当前调查时 `currentId` 要一起清空——指着一个不存在的 id 的话，
   * 快照里的 `case` 取不到，而 `currentIf` 会把旧界面发来的调用一路放行到 null 上。
   */
  forget(caseId: string) {
    this.drop(caseId);
    if (this.currentId === caseId) this.currentId = null;
  }
   ...
   * 但一个只是点开看过一眼的调查没有这个理由，全钉的话条数上限就等于不存在，
   * 调查列表会随着用过的调查无上限地长。
   */
   ...
   * 搜出来的那一行会把一个正卡在 `ask_operator` 上等人的调查显示成"已停止"——
  ...
  /**
   * 把库里查出来的一行合上运行时那一半。历史调查页用它——那一页不走 `briefs()`
   ...
   * 收掉只是丢运行时，调查本身在库里，点回去由 `create()` 重建。
   */
  private trimIdle() {
