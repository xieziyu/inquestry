 ...
 *
 * 这一次 spawn **与调查会话完全无关**、也没有任何要想的东西，因此四条纪律：
 *
 ...
 * - **关掉扩展思考**（`thinking: { type: 'disabled' }`）：模型侧的默认是 adaptive，
 *   而它会为一句标题想上三四十秒——同一段描述实测开着 41s、关掉 4.8s，正好跨在 `TIMEOUT_MS` 上。
 *   超时被下面那个 catch 收成"问不出来"，**失败形状与上一条一模一样**：标题静默停在兜底那句。
 *   这一趟只是把一段已经写好的文字压成一句短的，没有可想的东西
 *
...
export function namerOptions(model: string): Options {
  return {
    ...
    tools: [],
    // 见文件头第四条纪律：开着的话这一趟必超时，而超时长得跟"没起过"一样
