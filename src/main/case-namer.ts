 ...
 * 而这三处正是要一眼认出「是哪一次排查」的地方。所以标题由 agent 读完问题之后总结，
 * 立案那一刻先落的那句截断值只是个兜底（`titleOf`）。
 *
 * 这一次 spawn **与排查会话完全无关**，因此三条纪律：
 *
 ...
 * - **不给排查用的那套工具**：系统提示词是自备的一句话，不是 claude_code 预设
 *
...
const SYSTEM =
  '你在给一次线上问题排查起标题。只输出标题本身，不要引号、不要句号、不要任何解释或前后缀。' +
  `标题用中文，${MAX_TITLE} 字以内，写清「哪个系统 / 什么现象」，` +
  '不要写「排查」「问题」「分析」这类没有信息量的词，也不要编造描述里没有的服务名或结论。';
