 ...
 * 而 `case_seed_upload` 是唯一 `open` 且定稿闸通着的那份（形态选择器只有它点得到）。
 */
...
//
// **这一份是唯一点得到形态选择器的**：`status='open'`，而 impact 与 leftover 两步都已收好，
// 所以 `closingGaps` 是空的——报告屏点「定稿」会直接落出确认条，五档随便切。
// 切完不按「确认定稿」什么都不会变；真按了就冻上了，重跑这个脚本能还原。
//
...
function seedUploadCase() {
  ...
  // 两个强制 step 都收好 —— 这是这一份"点得到形态选择器"的全部条件
  const st4 = 'st_up_4';
  emit(C, s1, {
    type: 'step.opened',
