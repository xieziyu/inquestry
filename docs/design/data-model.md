...
**阶梯目前是空的**——v1→v5 每一级都动过 CHECK 约束或字段语义，那类只能重建。第一条真正 additive 的升级会是第一个走这条路的。空阶梯的行为与从前一模一样，所以这不是一条"没人验过的路"：`spike:cases` 拿一份**真跑出来的库**把版本号调低一格、给一级假步骤，整条走了一遍（案子留在原地 · 新列补上 · 版本跟着提 · 不留 `.bak`）。
