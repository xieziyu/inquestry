...
日志时间串大多**既无日期也无时区**（`12:03:01.220`）。没有基准日期与时区，`occurred_at_ms` 落不成绝对时刻，系统时间线就排不出来。所以它们**在新建排查时就落定**，不能等定稿（D27）。

| 字段 | 为什么 |
| --- | --- |
| `incident_date` / `tz_offset` | 上述基准，`NOT NULL`。**两项都不问人**，取立案那一刻的本机日期与偏移落库（理由与代价见 [ui](ui.md) §8.1）。重开旧排查时一律以库里那份为准：重算或改动都会让新证据与老证据错开 |
| `project_root` | agent 的运行目录（`Options.cwd`），决定它继承哪套 skill / MCP，也决定会话记录落盘位置。新建排查时必填。⚠️ **列本身仍可空**：这条规则之前立的排查有 `NULL`，读的那侧要接住 |
| `question` | 新建排查时写的完整问题。新开 session 时由它拼出首轮提问，基准日期也一并写进正文——harness 与 agent 两边补齐时间串的基准必须一致 |
| `clues` | 历史字段：面板不再单收「已知现象」（理由见 [ui](ui.md) §8.1）。旧排查里的值照旧读、照旧进首轮提问，新排查一律 `NULL` |
