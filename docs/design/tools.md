...
| 版本 | 规则 | 实测结果 |
| --- | --- | --- |
| v1 | 所有证据一律要求 `occurredAt` | ❌ **agent 拿查询执行时间凑数**，假时间直接混进事故时间线 |
...
`open_step` 带 `kind`（`normal` / `impact` / `leftover`）。提示词要求结案前有两个固定动作，但没有字段就落不进 `steps.kind`，schema 里的枚举永远填不满。

**`leftover` 的 `direction` 豁免可证伪性**：它是汇总，不是假设；提示词与校验都要明说，否则两边打架。

## 6.1 `close_step` 上的形态、应然实然与修复建议（D25 / D17）

`close_step` 有四个可选字段：`shape`（五种结论形态）、`expected` / `actual`、`remediation`。前三个**只在某一步给出了整个案子的根因时才填**，其余步一律留空。

`remediation` 不一样：它是报告四栏里唯一由 agent 生成的那一块（overview §6.1），**没查出根因时也该填**——那时填的是"下一步该怎么查、先加哪些观测"，落在汇总遗留疑点那一步上。它**只提醒不阻挡**：做成强制 step 要给 `steps.kind` 加一档，那就动了 CHECK 约束、不再是可重放的升级；而归档的残报告少这一栏本来也是诚实的。
...
- **"填了不生效"要当场回警告**，理由与 `occurredAt` 那三次收敛同源：不当场说，agent 以为已经交代过了，而报告要到结案那天才发现那一栏是空的。**静默忽略并不比错误采纳好多少。** 但警告**只陈述现状，不给处置**——一度写成「要让它算数，得先把那条根因推翻」，那等于教 agent 去推翻一条有效结论。该不该推翻只有查过的它自己判得了
