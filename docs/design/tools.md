...
`close_step` 有四个可选字段：`shape`（结论形态）、`expected` / `actual`、`remediation`。前三个**只在某一步给出了整个调查的根因时才填**，其余步一律留空。
...
**agent 的负担刻意压到"多填一个枚举"，但那一个是必填的**：形态由 agent 定，人不参与（D25 / [ui](ui.md) §8.4.2）。一度写着"拿不准就别填，不填由人选"，已随选择器一起推翻——**现在拿不准也要填**，因为判错不会让报告少一块（其余块照旧按各自门槛补，[overview](overview.md) §6.1.1），而空着会让报告落到 harness 的兜底推断上，那一档明写着"没人判断过"。

**`open` 不在可声明的枚举里**（`DECLARABLE_SHAPES`，定义在 `shared/ipc.ts`，`close_step` 的 zod 枚举直接用那一份）。它不是一次判断而是事实：一条已证实的根因都没有就是它，归档强制也是它。留在枚举里的话，agent 会拿它当"我没查出来"的表态，而那句话会盖掉库里一条真实存在的根因，让报告不印根因栏。读取侧对更早写进去的 `open` 声明一并当作没声明（`suggestVerdictShape()`）。
