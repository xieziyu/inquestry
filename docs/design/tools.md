...
`close_step` 后来又加了四个可选字段：`shape`（五种结论形态）、`expected` / `actual`、`remediation`。前三个**只在某一步给出了整个案子的根因时才填**，其余步一律留空。

`remediation` 不一样：它是报告四栏里唯一由 agent 生成的那一块（overview §6.1），**没查出根因时也该填**——那时填的是"下一步该怎么查、先加哪些观测"，落在汇总遗留疑点那一步上。四项同为 patch 语义：同一步再 close 一次，不重填就保持原样（补证据那一次多半只带 `evidence`），要改就重新填。

> `remediation` **只提醒不阻挡**：报告认定的根因刚落定、而全案一条建议都还没有时，`close_step` 回一句"这一栏目前是空的，重新 close 补上即可（只填这一项也行）"，补上就不再说。做成强制 step 要给 `steps.kind` 加一档，那就动了 CHECK 约束、不再是可重放的升级；而归档的残报告少这一栏本来也是诚实的。
