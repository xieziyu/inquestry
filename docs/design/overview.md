...
5. **立案面板 + schema v2** —— ✅ **已完成**（D27），见 [ui](ui.md) §8.1
   ...
   立案单落 `cases`（项目起点 / 基准日与时区 / 问题 / 已知线索），agent 三项落 `sessions`。三条实跑结论：
...
6. **待办栏两个控制手势 + 放开工具面** —— ✅ **已完成**，见 [ui](ui.md) §8.1 / §8.2，由 `npm run spike:gate` 兜底（15 条）

   ②档闸门（改写参数再放行 / deny + message 换方向）接上，真项目模式的工具面收成三档：直接放行 / 过闸门 / 一律不给。四条实跑结论：
...
7. **案件切换栏 + 全局待办汇总** —— ✅ **已完成**（D28），见 [ui](ui.md) §8.3，由 `npm run spike:cases` 兜底（16 条）

   main 改成同时持有多个案子的运行时（`src/main/case-registry.ts`），切换只换投影不中断任何一个。三条实跑结论：
...
8. **接下来**，按依赖排序。第一项做完是"日常能用起来"的最小闭环，之后才轮得到把报告做漂亮：

   1. **三种收尾**（D29）。`status` 的三档已在 schema 里，缺的是动作：终止时把挂起的 pending 一律改判 `abandoned`，否则库里会攒僵尸节点。
      ⚠️ **状态变更必须走领域事件**，不能直接 `UPDATE cases`：`cases.status` 目前没有对应事件，直接改的库值一重放就被抹回 `open`（`spike:cases` 的重放比对因此绕开了 status，那条注释里记了原因）
   2. **轨道改成主干 + 分叉**（D23）。当前是平铺卡片流；改成定宽画布内的绝对定位，加推翻回指线
   3. **写入 `verdict_shape` 与 `expected` / `actual`**（D25）。列已随 schema v2 落库，缺的是结案时的写入方——它决定报告装哪几块
   4. **报告拆成独立屏，改单列长页**（D21 / D22）。这步做完长图导出才有可能是对的
   5. **两种导出**（D26）。Markdown 先做（纯字符串拼装，可单测）；长图后做（要处理离屏窗口与字体就绪，见 [ui](ui.md) §7.2）
   6. **Spike A2（子 agent 泳道）**。分叉的真实数据来源还没验过——`agent_id` 与 `parent_tool_use_id` 是否够用，决定轨道里的支线怎么归属
