...
6. **接下来**，按依赖排序。前三项做完是"日常能用起来"的最小闭环，之后才轮得到把报告做漂亮：

   1. **待办栏接上两个控制手势**（改写参数再放行、deny + message 换方向）——控制面最值钱、也是唯一没接的部分。先做 ② 档闸门，再做 ① 档回填。做这一步时顺手把真项目模式的工具面放开：现在只给只读三件套，数据源类 MCP 全被拒
   2. **案件切换栏 + 全局待办汇总**（D28）。同时定活跃 session 上限与降级策略。**排查时间线目前按 session 取**，跨会话重开旧案时主区是空的，这一步一并解决
   3. **三种收尾**（D29）。`status` 的三档已在 schema 里，缺的是动作：终止时把挂起的 pending 一律改判 `abandoned`，否则库里会攒僵尸节点
   4. **轨道改成主干 + 分叉**（D23）。当前是平铺卡片流；改成定宽画布内的绝对定位，加推翻回指线
   5. **写入 `verdict_shape` 与 `expected` / `actual`**（D25）。列已随 schema v2 落库，缺的是结案时的写入方——它决定报告装哪几块
   6. **报告拆成独立屏，改单列长页**（D21 / D22）。这步做完长图导出才有可能是对的
   7. **两种导出**（D26）。Markdown 先做（纯字符串拼装，可单测）；长图后做（要处理离屏窗口与字体就绪，见 [ui](ui.md) §7.2）
   8. **Spike A2（子 agent 泳道）**。分叉的真实数据来源还没验过——`agent_id` 与 `parent_tool_use_id` 是否够用，决定轨道里的支线怎么归属
