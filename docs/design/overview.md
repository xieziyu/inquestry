...
> 首次成文：2026-08-10
> 状态：v0.1 可运行，界面规格已定（D21–D26），按 §9.5 推进
...
| D27 | 立案时必须收 **项目起点 + 事故基准日与时区**，落 `cases` 表 | 项目起点决定 agent 继承哪套 skill/MCP（§2）；基准日与时区决定 `occurred_at_ms` 能不能落成绝对时刻——没有它事故时间线就是空的。详见 [ui](ui.md) §8.1 |
...
5. **接下来**，按依赖排序。前四项是"日常能用起来"的最小闭环，之后才轮得到把报告做漂亮：

   1. **立案面板**（D27）。排第一不是因为它显眼，而是没有基准日就没有 `occurred_at_ms`，没有它事故时间线是空的。目前挂在会话上下文里，要落 `cases` 表
   2. **待办栏接上两个控制手势**（改写参数再放行、deny + message 换方向）——控制面最值钱、也是唯一没接的部分。先做 ② 档闸门，再做 ① 档回填
   3. **案件切换栏 + 全局待办汇总**（D28）。同时定活跃 session 上限与降级策略
   4. **三种收尾**（D29）。Case 加 `aborted`；终止时把挂起的 pending 一律改判 `abandoned`，否则库里会攒僵尸节点
   5. **轨道改成主干 + 分叉**（D23）。当前是平铺卡片流；改成定宽画布内的绝对定位，加推翻回指线
   6. **加 `verdict_shape` 与 `expected` / `actual`**（D25）。schema 增量很小但决定报告装哪几块，趁历史数据还少赶紧加——这类字段事后补的代价是历史案子全部残缺
   7. **报告拆成独立屏，改单列长页**（D21 / D22）。这步做完长图导出才有可能是对的
   8. **两种导出**（D26）。Markdown 先做（纯字符串拼装，可单测）；长图后做（要处理离屏窗口与字体就绪，见 [ui](ui.md) §7.2）
   9. **Spike A2（子 agent 泳道）**。分叉的真实数据来源还没验过——`agent_id` 与 `parent_tool_use_id` 是否够用，决定轨道里的支线怎么归属
