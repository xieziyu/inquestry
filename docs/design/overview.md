...
17. **接下来**：**依赖序上已经没有必做项了**（0–16 全部落地）。剩下的都在 [ui](ui.md) §12 那份欠缺清单里，按"不做会怎样"排，最靠前的三条：

    - **修复建议恒为「无」**：报告四栏里唯一由 agent 生成的那一块，而工具契约上还没有收它的字段（§6.1）。四栏缺一栏是报告本身不完整，这条最该先做
    - **旧案子只列不搜**：切换栏取最近 20 个，FTS5 跨 case 检索还没接上去（§8.3 / [data-model](data-model.md) §5）。案子攒到二十几个才会疼，但那一天一定会到
