...
两条线是同一批证据的两次投影（[data-model](data-model.md) §3），不是两份数据。事故时间线**不需要 agent 再写一遍**——它是 `ORDER BY occurredAt` 的结果。
...
一次事故经常跨多个会话（今天查一半明天接着查，或换个角度重开一轮），所以 session 之上必须有 Case 层。**检索维度是事故，不是会话。**
...
- **支线跑完由 harness 收口**：那一步落 `status='converged'`，正文取支线自己最后那句话。它不是一种判定——那一步没有命题——所以报告哪一栏都不进
...
> **「不投影」那一列与「主体块」同样是规则。** 这一带的错法只有一个形状：*有数据就装*——它一条都不报错，装出来的报告看着还更完整。归档强制未决型，而被归档的案子多半已经查出了点什么，按"有就装"写的话，明写着"没查出来"的残报告会顶着一条根因。
...
| D21 | 调查台与报告是**两个屏**，不是同屏两个 tab；但**共用同一套色板**，差别只来自密度与字号 | 人在两个屏前做的事不同，所以分屏；但一深一浅会被读成两个应用。详见 [ui](ui.md) §1 |
...
| D27 | 立案时必须收 **项目起点 + 事故基准日**（落 `cases`）以及 **backend / model / effort**（落 `sessions`） | 项目起点决定 agent 继承哪套 skill/MCP（§2）；基准日决定 `occurred_at_ms` 能不能落成绝对时刻。agent 三项放 session 层是因为一个案子跨多会话、中途换模型是常态。详见 [ui](ui.md) §8.1 |
...
| **仓库自带的配置是被信任的** | 真项目模式加载 `project` / `local` settings，其中的 `hooks` **加载即执行** shell、`.mcp.json` 直接拉起进程，三档工具面完全管不着（[ui](ui.md) §8.1 有实测） | 已决定接受：信任边界就是「立案时选的那个目录」，与在那儿直接跑 `claude` 等价 |
