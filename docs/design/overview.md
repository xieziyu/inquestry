...
8. **三种收尾** —— ✅ **已完成**（D29），见 [ui](ui.md) §8.4，由 `npm run spike:close` 兜底（36 条）

   停止 / 结案 / 归档三个动作接上，`cases.status` 有了写入方。schema 没动。四条实跑结论：
...
9. **接下来**，按依赖排序：
   ...
   1. **轨道改成主干 + 分叉**（D23）。当前是平铺卡片流；改成定宽画布内的绝对定位，加推翻回指线
   2. **写入 `verdict_shape` 与 `expected` / `actual`**（D25）。列已随 schema v2 落库，缺的是结案时的写入方——它决定报告装哪几块。**结案那一步现在是它的落点**：收尾动作已经在了，形态枚举挂上去即可
   3. **报告拆成独立屏，改单列长页**（D21 / D22）。这步做完长图导出才有可能是对的
   4. **两种导出**（D26）。Markdown 先做（纯字符串拼装，可单测）；长图后做（要处理离屏窗口与字体就绪，见 [ui](ui.md) §7.2）。**归档的残报告在这一步才真的能导**——收尾只保证了证据没被销毁
   5. **Spike A2（子 agent 泳道）**。分叉的真实数据来源还没验过——`agent_id` 与 `parent_tool_use_id` 是否够用，决定轨道里的支线怎么归属
