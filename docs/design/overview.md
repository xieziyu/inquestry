...
5. **接下来**，按此顺序：
   ...
   1. **待办栏接上两个控制手势**（改写参数再放行、deny + message 换方向）——控制面最值钱、也是唯一没接的部分。先做 ② 档闸门，再做 ① 档回填
   2. **轨道改成主干 + 分叉**（D23）。当前是平铺卡片流；改成定宽画布内的绝对定位，加推翻回指线
   3. **加 `verdict_shape` 与 `expected` / `actual`**（D25）。schema 增量很小但决定报告装哪几块，趁历史数据还少赶紧加——这类字段事后补的代价是历史案子全部残缺
