...
10. **接下来**，按依赖排序：
    ...
    2. **报告拆成独立屏，改单列长页**（D21 / D22）。按 `verdict_shape` 组装的块现在有数据了，这步做完长图导出才有可能是对的
    3. **两种导出**（D26）。Markdown 先做（纯字符串拼装，可单测）；长图后做（要处理离屏窗口与字体就绪，见 [ui](ui.md) §7.2）。**归档的残报告在这一步才真的能导**——收尾只保证了证据没被销毁
    4. **Spike A2（子 agent 泳道）**。分叉的真实数据来源还没验过——`agent_id` 与 `parent_tool_use_id` 是否够用，决定轨道里的支线怎么归属
