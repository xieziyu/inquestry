...
- **case**：主工作台——Timeline 控制面 + 输入带
- **report**：报告预览与导出（Markdown + mermaid）

### 两条时间线：先做同屏切换，不做两栏并置

overview D2（Timeline 为主）标记为待体验验证。两条线的呈现选**同一屏视图切换**：

- 事故时间线在排查早期基本是空的，两栏并置会有一半时间是空屏
- 切换形态更容易验证 D2 到底顺不顺手——这正是要验的东西

**布局可以换，数据模型换不了**（overview §8.1）。切换与并置之间的改动是前端局部，不影响 EvidenceRef 的双时间戳设计。
